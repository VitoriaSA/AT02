...
class ModConfig{
    ...
    public function get_data($prefix = '', $admin_only = 0){
        $cond = array('admin_only' => $admin_only);
        $this->db->like('nome', $prefix);
        $rs = $this->db->get_where('config', $cond);
        return $rs->result();
    }
     ...
     */
    public function insert_or_update($table, $data) {
